...
import numpy as np
from fem_analogy_2d_utilities import setup_fem_beam_analogy, map_forces_to_nodes, check_resultant 
...
for i in range(nr_coordinates):
    model_part.CreateNewNode(i+1,np.random.uniform(up,low),np.random.uniform(up,low),0)

# creating nodal coordinates for the process syntax
nodal_coordinates = []
for node in model_part.Nodes:
    nodal_coordinates.append(np.array([node.X0, node.Y0]))
...
# loop over each target force entry
for i in range(nr_target_resultants):
    print("\nTarget force iteration: " + str(i))
    target_resultants = np.array([np.random.uniform(up,low),    #x
                                  np.random.uniform(up,low),    #y
                                  np.random.uniform(up,low)])   #rotation around z
    ...
    for node in model_part.Nodes:
        node.SetSolutionStepValue(KSM.POINT_LOAD_X,nodal_forces[2*(node.Id-1)+0])
        node.SetSolutionStepValue(KSM.POINT_LOAD_Y,nodal_forces[2*(node.Id-1)+1])
    ...
    for node in model_part.Nodes:
        ...
        mf[0] += x * nodal_force[1] - y * nodal_force[0]
